...
*/
       ...
       bzero(recmsg, 11);
       n = recvfrom(socket_fd, recmsg, 11, 0,
               (struct sockaddr *) &group_addr, &addr_len);
       if (n < 0) {
           printf("recvfrom err in udptalk!\n");
           exit(4);
           ...
           // success recieve the information 
           recmsg[n] = 0;
           printf("peer:%02x,%02x,%02x,%02x,%02x,%02x  \n", (unsigned char)recmsg[4], (unsigned char)recmsg[5], (unsigned char)recmsg[6],(unsigned char)recmsg[7], (unsigned char)recmsg[8], (unsigned char)recmsg[9] );
       }
